...
- **fixedVolume (boolean, facultative)**: Specify if the rigid body volume is allowed to change throughout the simulation. If true, then the volume won't change and volumeComputation tag becomes mandatory. Default is false.
...
- **volumeComputation (string, semi-mandatory)**: Specify the technique used to compute the volume of the rigid body. It isn't case sensitive and the accepted values are "None" (default), "Auto" and "TriangleIntegration". Note that no matter the choice, volume computation will be disabled for individual particles.
