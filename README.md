...
- **geometry (tag, semi-facultative)**: Geometry type of the rigidbody. It isn’t case sensitive and the accepted values are “Sphere”, “Cube” and "EquiSphereMDeserno". Only available (and mandatory) if we chose Uniform as layering generation technique. "EquiSphereMDeserno" is also a uniform sphere, but the sampling is done using M. Deserno algorithm given in its paper. This tag sould be accompanied by some attributes depending on the chosen type :
