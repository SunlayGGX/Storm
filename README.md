...
- **Partio v1.14.6** (link: https://github.com/wdas/partio/releases/tag/v1.14.6). 
 + Clone the repo
 + use CMake to generate solution folder. Note : If you don't have Glut, we don't need it because we use the library just as an exporter, therefore you can comment every line related to Opengl or Glut on CMakeList.txt. That is not mandatory and if you have those lib already installed, don't bother, you can build with or without.
 + Change the configuration of the runtime to Multi-threaded Debug (/MTd) for Debug and Multi-threaded (/MT) for Release.
 + build only the partio project on Release and Debug. Though you can build them all, but we only need partio project.
...
- **displayRbInFull (boolean, facultative)**: Specify if Solids should be displayed in full (ignoring near and far planes by being rendered in a pre pass). “false” by default.
...
- **reducedMassCoeff (positive float tag, facultative)**: This is the coefficient for mass initialization to prevent preessure spikes at the start of the simulation (cf SPlisHSPlasH). Default is 0.8. It can have attributes defined as follow :
 + **startValue (positive float, facultative)**: This is starting the coefficient for the mass initialization coefficient. If unset, the coefficient will be directly reducedMassCoeff.
 + **fadeInTime (positive float, facultative)**: This is transition duration (in seconds) from the starting coefficient to the final coefficient. Default is 1 second. Ignored if startValue is unset.
 + **updateMass (boolean, facultative)**: Specify if true that we should update fluid particle masses and not just the volume. Default is false. Ignored if startValue is unset.
...
- **geometry (string tag, semi-facultative)**: Geometry type of the rigidbody. It isn’t case sensitive and the accepted values are “Sphere”, "SphereAlKashi", “Cube” and "EquiSphereMDeserno". Only available (and mandatory) if we chose Uniform as layering generation technique. "EquiSphereMDeserno" is also a uniform sphere, but the sampling is done using M. Deserno algorithm given in its paper. "Sphere_AlKashi" is an equisphere but we use the Al Kashi theorem to enforce the distance between particle is not less than the separation distance (instead of an approximation given by a rule of thumbs). This tag should be accompanied by some attributes that would be used depending on the chosen type :
 + **sampleCountMDeserno (positive integer, semi-facultative)**: This is the input sample count of M. Deserno sampling algorithm of a sphere. It should be greater than 0. It is mandatory if specified geometry type is "EquiSphereMDeserno".
 + **openXLeft (boolean, facultative)**: When geometry type is "Cube" and this flag is true, then the left face of the Cube will be removed. Default is false (the face would be present).
 + **openXRight (boolean, facultative)**: When geometry type is "Cube" and this flag is true, then the right face of the Cube will be removed. Default is false (the face would be present).
 + **openYUp (boolean, facultative)**: When geometry type is "Cube" and this flag is true, then the top face of the Cube will be removed. Default is false (the face would be present).
 + **openYDown (boolean, facultative)**: When geometry type is "Cube" and this flag is true, then the bottom face of the Cube will be removed. Default is false (the face would be present).
 + **openZFront (boolean, facultative)**: When geometry type is "Cube" and this flag is true, then the front face of the Cube will be removed. Default is false (the face would be present).
 + **openZBack (boolean, facultative)**: When geometry type is "Cube" and this flag is true, then the back face of the Cube will be removed. Default is false (the face would be present).
...
- **applyVortice (boolean, facultative)**: This flag specifies if the blower should make vertices. Default is false.
- **vorticeCoeff (positive float, facultative)**: It is a positive scaling coefficient of any force created as vorticity from the blower. It is ignored if the feature is disabled, otherwise its default value is 1.0.
...
- **void logForceParticipationOnTotalForce(const unsigned int id, const Storm::CustomForceSelect force, const int pressureMode)**: Log the participation of the specified force on particle system referred with id. Custom force would always be all forces of the type no matter which CustomForceSelect we chose between the unary force or the multi. In case of pressure, pressureMode should be equal to 0 is we want to log the final pressure force, 1 if we want the temporary density pressure force and 2 if we want the temporary velocity pressure force. Any other value is forbidden and this flag is ignored if another force is selected.
- **void logForceParticipationOnVector(const unsigned int id, const Storm::CustomForceSelect force, const int pressureMode, float x, float y, float z)**: Log the participation of the specified force on particle system referred with id to the vector those coordinate components are { x, y, z }. Except this fact, this method obeys the same rules as logForceParticipationOnTotalForce method.
- **void logForceParticipationToEnterBlower(const unsigned int rbId, const unsigned int blowerId, const Storm::CustomForceSelect force, const int pressureMode)**: Log the participation of the specified force on particle system referred with rbId to the force vector of the blower referred with blowerId. Except this fact, this method obeys the same rules as logForceParticipationOnTotalForce method.
...
- **void forceRefreshParticleNeighborhood(const unsigned int id, const std::size_t pIndex)**: Force the neighborhood for the specified pIndex-th particle from the particle system with id to be refreshed. WARNING: This is only for debugging/troubleshooting purpose (step into, ...). This method could be left trailing behind recent neighborhood building change in the codebase. We'll update it only when we need to debug the feature!
...
- **void stopTrackingTranslationOnWatchedRb()**: Stop tracking the rigidbody translation. This does not clear the rigidbody tracking (we would still move the near plane).
...
- **void displayDynamicRbInFull(bool enable)**: Display rigidbodies (except walls) without near plane cut.
