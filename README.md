...
- **applyDragEffectOnFluid (boolean, facultative)**: Enable fluid to fluid drag effects. Default is false.
...
- **normalsCoherency (boolean, facultative)**: if true, a second pass will be made to ensure normals are coherents (point to the same way (to the interior if it is a wall, or to the exterior if it is an non-wall object)). It is better to keep this setting to false for concave or miscellaneous objects. Default is true.
