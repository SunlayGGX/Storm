...
- **geometry (tag, semi-facultative)**: Geometry type of the rigidbody. It isn’t case sensitive and the accepted values are “Sphere”, "SphereAlKashi", “Cube” and "EquiSphereMDeserno". Only available (and mandatory) if we chose Uniform as layering generation technique. "EquiSphereMDeserno" is also a uniform sphere, but the sampling is done using M. Deserno algorithm given in its paper. "Sphere_AlKashi" is an equisphere but we use the Al Kashi theorem to enforce the distance between particle is not less than the separation distance (instead of an approximation given by a rule of thumbs). This tag should be accompanied by some attributes depending on the chosen type :
