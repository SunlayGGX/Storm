...
#undef STORM_SOLVER_NAMES_XMACRO
	...
	Storm::Vector3 computeViscosity(const Storm::IterationParameter &iterationParameter, const Storm::SceneFluidConfig &fluidConfig, const Storm::FluidParticleSystem &fluidParticleSystem, const std::size_t currentPIndex, const float currentPMass, const Storm::Vector3 &vi, const float currentPDensity, const float viscoPrecoeff)
	{
		...
		Storm::FluidParticleSystemUtils::forEachNeighbor(fluidParticleSystem, currentPIndex, [&]<Storm::FluidParticleSystemUtils::NeighborType neighborType>(const Storm::NeighborParticleInfo & neighbor)
		{
			...
			else
			{
				...
				if (rbViscosity > 0.f)
				{
					const Storm::Vector3 vij = vi - neighborPSystemAsBoundary->getVelocity()[neighbor._particleIndex];
					...
					{
						Storm::Vector3 &boundaryNeighborTmpViscosityForce = neighborPSystemAsBoundary->getTemporaryViscosityForces()[neighbor._particleIndex];
	...
	Storm::Vector3 computeBernouilliPrinciple(const Storm::FluidParticleSystem &fluidParticleSystem, const std::size_t currentPIndex)
	{
		// Explanation :
		...
		Storm::FluidParticleSystemUtils::forEachNeighbor(fluidParticleSystem, currentPIndex, [&]<Storm::FluidParticleSystemUtils::NeighborType neighborType>(const Storm::NeighborParticleInfo & neighbor)
		{
			...
			else
			{
				...
				if constexpr (neighborType == Storm::FluidParticleSystemUtils::NeighborType::DynamicRb)
				{
					Storm::Vector3 &boundaryNeighborTmpDynamicQForce = neighborPSystemAsBoundary->getTemporaryBernoulliDynamicPressureForces()[currentPIndex];
	...
	Storm::Vector3 produceNoStickConditionForces(const Storm::IterationParameter &iterationParameter, const Storm::FluidParticleSystem &fluidParticleSystem, const Storm::Vector3 &vi, const std::size_t currentPIndex)
	{
		Storm::Vector3 result = Storm::Vector3::Zero();

		const float currentPMass = fluidParticleSystem.getMasses()[currentPIndex];

		Storm::FluidParticleSystemUtils::forEachRigidbodyNeighbor(fluidParticleSystem, currentPIndex, [&]<Storm::FluidParticleSystemUtils::NeighborType neighborType>(const Storm::NeighborParticleInfo & neighbor)
		{
			STORM_STATIC_ASSERT(neighborType != Storm::FluidParticleSystemUtils::NeighborType::Fluid, "This method shouldn't be called on fluids!");
...
Storm::DFSPHSolverModified::DFSPHSolverModified(const float k_kernelLength, const Storm::ParticleSystemContainer &particleSystemsMap) :
...
void Storm::DFSPHSolverModified::removeRawEndData(const unsigned int pSystemId, std::size_t toRemoveCount)
{
	Storm::SPHSolverUtils::removeRawEndData(pSystemId, toRemoveCount, _data);
}

void Storm::DFSPHSolverModified::setEnableThresholdDensity(bool enable)
{
	_enableThresholdDensity = enable;
}

void Storm::DFSPHSolverModified::setUseRotationFix(bool useFix)
{
	_useRotationFix = useFix;
}

void Storm::DFSPHSolverModified::setNeighborThresholdDensity(std::size_t neighborCount)
{
	_neighborThresholdDensity = neighborCount;
}


void Storm::DFSPHSolverModified::initializeStepDensities(const Storm::IterationParameter &iterationParameter, const Storm::SceneSimulationConfig &sceneSimulationConfig, const Storm::SceneFluidCustomDFSPHConfig &sceneDFSPHSimulationConfig)
{
	...
	if (sceneDFSPHSimulationConfig._useBernoulliPrinciple)
	{
		for (auto &particleSystemPair : particleSystems)
		{
			...
			if (currentParticleSystem.isFluids())
			{
				...
				Storm::runParallel(fluidPSystem.getPressures(), [this, &fluidPSystem, &velocities, &densities, density0 = fluidPSystem.getRestDensity()](float &currentPQ, const std::size_t currentPIndex)
				{
					const Storm::Vector3 &vi = velocities[currentPIndex];
					...
					Storm::FluidParticleSystemUtils::forEachNeighbor(fluidPSystem, currentPIndex, [&]<Storm::FluidParticleSystemUtils::NeighborType neighborType>(const Storm::NeighborParticleInfo &neighbor)
					{
						// Fluids
						if constexpr (neighborType == Storm::FluidParticleSystemUtils::NeighborType::Fluid)
						{
							...
							const Storm::Vector3 &vj = neighborPSystemAsFluid->getVelocity()[neighbor._particleIndex];
...
void Storm::DFSPHSolverModified::fullDensityInvariantSolve_Internal(const Storm::IterationParameter &iterationParameter, const Storm::SceneFluidCustomDFSPHConfig &sceneDFSPHSimulationConfig)
{
...
void Storm::DFSPHSolverModified::fullVelocityDivergenceSolve_Internal(const Storm::IterationParameter &iterationParameter, const Storm::SceneFluidCustomDFSPHConfig &sceneDFSPHSimulationConfig)
{
...
void Storm::DFSPHSolverModified::computeNonPressureForces_Internal(const Storm::IterationParameter &iterationParameter, const Storm::SceneSimulationConfig &sceneSimulationConfig, const Storm::SceneFluidConfig &fluidConfig)
{
	...
	for (auto &particleSystemPair : particleSystems)
	{
		...
		if (currentParticleSystem.isFluids())
		{
			...
			Storm::DFSPHSolverModified::DFSPHSolverDataArray &dataField = _data.find(particleSystemPair.first)->second;
...
#undef STORM_COMPUTE_VISCOSITY
				...
				if (dfsphFluidConfig._useBernoulliPrinciple)
				{
					...
					dynamicPressureForce = computeBernouilliPrinciple(fluidParticleSystem, currentPIndex);
				...
				if (sceneSimulationConfig._noStickConstraint)
				{
					...
					currentPTmpNoStickForce = produceNoStickConditionForces(iterationParameter, fluidParticleSystem, vi, currentPIndex);
...
void Storm::DFSPHSolverModified::divergenceSolve(const Storm::IterationParameter &iterationParameter, const Storm::SceneFluidCustomDFSPHConfig &sceneDFSPHSimulationConfig, unsigned int &outIteration, float &outAverageError)
{
	const Storm::SingletonHolder &singletonHolder = Storm::SingletonHolder::instance();
	const Storm::IConfigManager &configMgr = singletonHolder.getSingleton<Storm::IConfigManager>();
	...
	do
	{
		...
		for (auto &dataFieldPair : _data)
		{
			...
			Storm::runParallel(dataFieldPair.second, [&](Storm::DFSPHSolverData &currentPData, const std::size_t currentPIndex)
			{
				...
				const Storm::DFSPHSolverModified::DFSPHSolverDataArray* neighborDataArray = &dataFieldPair.second;
				...
				}, std::fabs(ki) > Storm::SPHSolverPrivateLogic::k_epsilon);
			});
...
void Storm::DFSPHSolverModified::pressureSolve(const Storm::IterationParameter &iterationParameter, const Storm::SceneFluidCustomDFSPHConfig &sceneDFSPHSimulationConfig, unsigned int &outIteration, float &outAverageError)
{
	const Storm::SingletonHolder &singletonHolder = Storm::SingletonHolder::instance();
	const Storm::IConfigManager &configMgr = singletonHolder.getSingleton<Storm::IConfigManager>();
...
void Storm::DFSPHSolverModified::computeDFSPHFactor(const Storm::IterationParameter &iterationParameter, Storm::FluidParticleSystem &fluidPSystem, Storm::DFSPHSolverModified::DFSPHSolverDataArray &pSystemData, const double kMultiplicationCoeff)
{
	...
	//////////////////////////////////////////////////////////////////////////
	Storm::runParallel(pSystemData, [&iterationParameter, &fluidPSystem, kMultiplicationCoeff](Storm::DFSPHSolverData &currentPData, const std::size_t currentPIndex)
	{
		//////////////////////////////////////////////////////////////////////////
		...
		Storm::FluidParticleSystemUtils::forEachNeighbor(fluidPSystem, currentPIndex, [&sum_grad_p_k, &grad_p_i]<Storm::FluidParticleSystemUtils::NeighborType neighborType>(const Storm::NeighborParticleInfo & neighbor)
		{
			// Fluids
			if constexpr (neighborType == Storm::FluidParticleSystemUtils::NeighborType::Fluid)
			{
...
void Storm::DFSPHSolverModified::computeDensityAdv(const Storm::IterationParameter &iterationParameter, Storm::FluidParticleSystem &fluidPSystem, const Storm::DFSPHSolverModified::DFSPHSolverDataArray* currentSystemData, Storm::DFSPHSolverData &currentPData, const std::size_t currentPIndex)
{
	...
	const Storm::DFSPHSolverModified::DFSPHSolverDataArray* neighborDataArray = currentSystemData;
	const Storm::FluidParticleSystem* lastNeighborFluidSystem = &fluidPSystem;

	float delta = 0.f;

	Storm::FluidParticleSystemUtils::forEachNeighbor(fluidPSystem, currentPIndex, [&vi = currentPData._predictedVelocity, &delta, &neighborDataArray, &lastNeighborFluidSystem, this]<Storm::FluidParticleSystemUtils::NeighborType neighborType>(const Storm::NeighborParticleInfo & neighbor)
	{
		// Fluids
		if constexpr (neighborType == Storm::FluidParticleSystemUtils::NeighborType::Fluid)
		{
		...
		{
			const Storm::RigidBodyParticleSystem*const neighborPSystemAsBoundary = static_cast<const Storm::RigidBodyParticleSystem*>(neighbor._containingParticleSystem);
			const Storm::Vector3 &vj = neighborPSystemAsBoundary->getVelocity()[neighbor._particleIndex];
...
void Storm::DFSPHSolverModified::computeDensityChange(const Storm::IterationParameter &iterationParameter, Storm::FluidParticleSystem &fluidPSystem, const Storm::DFSPHSolverModified::DFSPHSolverDataArray* currentSystemData, Storm::DFSPHSolverData &currentPData, const std::size_t currentPIndex)
{
	...
	if (!_enableThresholdDensity || currentPNeighborhood.size() >= _neighborThresholdDensity)
	{
		...
		Storm::FluidParticleSystemUtils::forEachNeighbor(fluidPSystem, currentPIndex, [&lastNeighborFluidSystem, &neighborDataArray, &vi = currentPData._predictedVelocity, &densityAdv, this]<Storm::FluidParticleSystemUtils::NeighborType neighborType>(const Storm::NeighborParticleInfo & neighbor)
		{
			// Fluids
			if constexpr (neighborType == Storm::FluidParticleSystemUtils::NeighborType::Fluid)
			{
