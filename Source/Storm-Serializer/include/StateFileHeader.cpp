...
void Storm::StateFileHeader::endSerialize(Storm::SerializePackage &package)
{
	...
	{
		Storm::throwException<std::exception>(__FUNCTION__ " shouldn't be used when loading the state file!");
	}
}
...
void Storm::StateFileHeader::serializePreheader(Storm::SerializePackage &package, bool end)
{
	...
	else
	{
		...
		{
			Storm::throwException<std::exception>("Preheader serializer checksum is invalid. Something went wrong when the state file was created and it was never completed!");
		}
