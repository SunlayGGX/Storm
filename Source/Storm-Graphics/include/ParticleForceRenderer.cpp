...
void Storm::ParticleForceRenderer::refreshForceData(const ComPtr<ID3D11Device> &device, const Storm::GraphicParameters &params)
{
	const Storm::Vector3 force[2] =
	{
		_lastPositionCached, // Origin
		_lastPositionCached + (_lastForceCached * params._vectNormMultiplicator) // End of the vector arrow
	};
...
void Storm::ParticleForceRenderer::updateForceData(const ComPtr<ID3D11Device> &device, const Storm::GraphicParameters &params, const Storm::Vector3 &selectedPosition, const Storm::Vector3 &selectedForce)
{
	if (_lastPositionCached != selectedPosition || _lastForceCached != selectedForce || _vertexBuffer == nullptr)
	{
		...
		this->refreshForceData(device, params);
	}
}
